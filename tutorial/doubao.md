...
::: warning ⚠️ 关键提醒：检查免费额度与账户状态
获取 API Key 和接入点后，请在火山引擎方舟控制台确认账户处于正常状态（未欠费），且所选模型的免费额度正常生效（火山方舟通常为新接入模型赠送 50万 Tokens 免费额度）。  
若账户欠费或无可用 Token 额度，API 请求将无法成功响应，导致 App 内生成 AI 报告失败。
:::
